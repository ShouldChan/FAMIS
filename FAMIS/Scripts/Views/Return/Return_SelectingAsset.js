...
function load_SC_Tree() {
    ...
    $('#lefttree').tree({
        ...
        url: '/Dict/loadSearchTreeByRole?treeType=ReturnSearch',
...
function LoadInitData_Detail() {
    ...
    $('#tableList_return').datagrid({
        url: '/Return/LoadAsset_ByState?stateID=4&searchCondtiion=' + searchCondtiion + "&selectedIDs=" + _list,
        ...
        columns: [[
            ...
            { field: 'serialNum_JC', title: '借出单号', width: 50 },
...
function loadPageTool_Detail() {
    var pager = $('#tableList_return').datagrid('getPager');	// get the pager of datagrid
    pager.pagination({
        buttons: [{
            text: '刷新',
            height: 50,
            iconCls: 'icon-reload',
            handler: function () {
                $('#tableList_return').datagrid('reload');
                //alert('刷新');
            }
        }, {
            text: '选择资产',

//====================================Global Data==================================//
var CurrentList = new Array();  //当前添加的数据
var searchCondtiion = "";
//====================================Global Data==================================//
...
function load_SC_Tree() {
    ...
    $('#lefttree').tree({
        ...
        url: '/Dict/loadSearchTreeByRole?treeType=ReductionSearch',
...
function LoadInitData_Detail() {
    ...
    $('#tableList_reduction').datagrid({
        url: '/Common/LoadAsset_ByState?stateID=2_1_4_5&searchCondtiion=' + searchCondtiion + "&selectedIDs=" + _list,
...
function loadPageTool_Detail() {
    var pager = $('#tableList_reduction').datagrid('getPager');	// get the pager of datagrid
    pager.pagination({
        buttons: [{
            text: '刷新',
            height: 50,
            iconCls: 'icon-reload',
            handler: function () {
                $('#tableList_reduction').datagrid('reload');
                //alert('刷新');
            }
        }, {
            text: '选择资产',
            ...
            handler: function () {
              //获取选择中的行数
                //获取选择行
                var rows = $('#tableList_reduction').datagrid('getSelections');
                var IDS = [];

...
function load_SC_Tree() {
    ...
    $('#lefttree').tree({
        ...
        url: '/Dict/loadSearchTreeByRole?treeType=BorrowSearch',
...
function loadPageTool_Detail() {
    var pager = $('#tableList_borrow').datagrid('getPager');	// get the pager of datagrid
    pager.pagination({
        buttons: [{
            text: '刷新',
            height: 50,
            iconCls: 'icon-reload',
            handler: function () {
                $('#tableList_borrow').datagrid('reload');
                //alert('刷新');
            }
        }, {
            text: '选择资产',
            ...
            handler: function () {
              //获取选择中的行数
                //获取选择行
                var rows = $('#tableList_borrow').datagrid('getSelections');
                var IDS = [];
